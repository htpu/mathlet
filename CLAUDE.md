...
mathlet — 728 interactive math formula visualizations. Vite + TS strict + KaTeX + Three.js, custom 80-LOC reactive runtime, SSG to Cloudflare Pages.
...
npm run build      # registry → vite build → ssg (prerenders index + 728 detail pages + sitemap)
...
## Domain list (23)
`algebra`, `geometry`, `calculus`, `linalg`, `ode`, `pde`, `probability`, `fractal`, `topology`, `numbertheory`, `signals`, `optimization`, `vectorfield`, `cellular`, `biology`, `chemistry`, `quantum`, `graph`, `crypto`, `music`, `gr`, `astronomy`, `economics`.
