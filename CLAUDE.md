...
mathlet — 740 interactive math formula visualizations. Vite + TS strict + KaTeX + Three.js, custom 80-LOC reactive runtime, SSG to Cloudflare Pages.
...
npm run build      # registry → vite build → ssg (prerenders index + 740 detail pages + sitemap)
...
## Domain list (27)
`algebra`, `geometry`, `calculus`, `linalg`, `ode`, `pde`, `probability`, `fractal`, `topology`, `numbertheory`, `signals`, `optimization`, `vectorfield`, `cellular`, `biology`, `chemistry`, `quantum`, `graph`, `crypto`, `music`, `gr`, `astronomy`, `economics`, `statmech`, `fluid`, `chaos`, `gametheory`.
