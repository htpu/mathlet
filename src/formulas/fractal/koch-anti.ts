...
export default {
  meta:{slug:'koch-anti',title:'反向 Koch（向内）',domain:'fractal',level:3,tex:'\\text{indent instead of out}',blurb:'每段向内凹折 → 反 Koch。'},
  ...
  render(s,p){
    ...
    const sc=Math.min(width,height)*0.7;
    const ox=(width-sc)/2, oy=(height-sc)/2;
    ctx.strokeStyle='#f07178'; ctx.lineWidth=1.2;
    ctx.beginPath();
