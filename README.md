...
**725 interactive math formula visualizations** — every formula is a live canvas with adjustable parameters. 22 domains. Zero framework. Vite + TypeScript strict + KaTeX + Three.js, custom 80-LOC reactive runtime, SSG to Cloudflare Pages.
...
- **725 formulas** across 22 domains: algebra, geometry, calculus, linear algebra, ODE, PDE, probability, fractal, topology, number theory, signals, optimization, vector field, cellular automata, biology, chemistry, quantum, graph, crypto, music, general relativity.
...
- **i18n**: 中 / EN / ES with full coverage on all 725 entries.
...
npm run build      # registry → vite build → ssg (725 detail pages + 28 landings + sitemap)
...
- **`src/main-index.ts`** — index page: featured row + 22 domain mosaic tiles + filter chips + breadcrumbs.
